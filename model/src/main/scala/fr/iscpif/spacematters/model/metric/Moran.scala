...
object Moran {
  ...
  def moran[T](state: Matrix[T], quantity: Quantity[T], neighbors: Matrix[T] => Seq[(T, T, Double)]): Double = {
    ...
    val totalWeight = pairs.map { case (_, _, weight) => weight }.sum
